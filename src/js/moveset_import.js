...
function getStats(currentPoke, rows, offset) {
	...
	for (var x = offset; x < offset + 9; x++) {
		...
		if (!currentPoke.data.location && currentLocation[0] == "Location") {
			currentPoke.data.location = currentLocation[1].trim().replace(" (NPC)", "");
		}
...
function addToDex(poke) {
	...
	var base = SPECIES[toID(poke.name)];
	if (base.baseSpecies) base = SPECIES[base.baseSpecies];
	while (base.prevo) base = SPECIES[base.prevo];

	if (dexObject.data) for (var i in customsets) {
		...
		for (var j in customsets[i]) {
			...
			if (set.data.id && set.data.id == dexObject.data.id && species.id == base.id && (poke.name !== i || poke.nameProp !== j)) {
				removeSet(`${i} (${j})`);
				delete customsets[i][j];
			}
		}
	}
...
$(document).ready(function () {
	...
	if (localStorage.customsets) {
		...
		for (var i in customSets) {
			for (var j in customSets[i]) {
				if (!customSets[i][j].data) customSets[i][j].data = {id: "", location: customSets[i][j].location, dead: customSets[i][j].dead == true};
				if ("dead" in customSets[i][j]) {
					customSets[i][j].data.dead = customSets[i][j].dead;
					delete customSets[i][j].dead;
				}
			}
		}
		updateDex(customSets);
