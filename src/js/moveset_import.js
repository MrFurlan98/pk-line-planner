...
function parseSaveFile(data) {
	...
	for (var i = 0; i < 420; i++) {
		if (addSavePokemon(pc.slice(i * 136, (i + 1) * 136))) added++;
	}
...
$(document).ready(function () {
	...
	if (localStorage.customsets) {
		...
		for (var i in customSets) {
			for (var j in customSets[i]) {
				if (!customSets[i][j].data) customSets[i][j].data = {id: "", location: customSets[i][j].location};
			}
		}
		updateDex(customSets);
		applyIconColors();
